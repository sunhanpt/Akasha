...
namespace aka
{
	namespace core
	{
		...
		*/
		...
		template <typename T, typename TAlloc = akaAllocator<T> >
		class string;
		...
		static eLocaleID locale_current = AKA_LOCALE_ANSI;
		...
		static inline u32 locale_lower(u32 x)
		{
			...
			{
			case AKA_LOCALE_GERMAN:
			case AKA_LOCALE_ANSI:
				break;
			}
			// ansi
		...
		*/
		AKASA_API void utf8ToWchar(const char *in, wchar_t *out, const u64 len);

		//! Convert this wchar string to utf-8.
		/** The resulting string is always NULL-terminated and well-formed.
		\param len The size of the output buffer in bytes.
		*/
		AKASA_API void wcharToUtf8(const wchar_t *in, char *out, const u64 len);

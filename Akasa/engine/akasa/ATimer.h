...
#ifndef __A_AKA_C_TIMER_H_INCLUDED__
#define __A_AKA_C_TIMER_H_INCLUDED__
...
namespace aka
{
	...
	public:
		...
		virtual u32 getRealTime() const _AKA_OVERRIDE_
		{
			return os::Timer::getRealTime();
		}

		//! Get current time and date in calendar form
		virtual RealTimeDate getRealTimeAndDate() const _AKA_OVERRIDE_
		{
		...
		virtual u32 getTime() const _AKA_OVERRIDE_
		{
			return os::Timer::getTime();
		}

		//! sets current virtual time
		virtual void setTime(u32 time) _AKA_OVERRIDE_
		{
		...
		corretly again. */
		virtual void stop() _AKA_OVERRIDE_
		{
			os::Timer::stopTimer();
		}
		...
		stopTimer() will also have to call startTimer(), otherwise the timer may not start/stop
		corretly again. */
		virtual void start() _AKA_OVERRIDE_
		{
			os::Timer::startTimer();
		}
		...
		real system time. */
		virtual void setSpeed(f32 speed = 1.0f) _AKA_OVERRIDE_
		{
		...
		/** The speed is the factor with which the time is running faster or slower then the
		real system time. */
		virtual f32 getSpeed() const _AKA_OVERRIDE_
		{
			return os::Timer::getSpeed();
		}

		//! Returns if game timer is currently stopped
		virtual bool isStopped() const _AKA_OVERRIDE_
		{
		...
		virtual void tick() _AKA_OVERRIDE_
		{
			os::Timer::tick();
		}
	};

...
// what level of checking to perform...normally checkSlow but could be ensure or assert
#define checkThreadGraph assert
...
namespace ENamedThreads
{
	...
	__forceinline int GetQueueIndex(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & QueueIndexMask) >> QueueIndexShift;
	}

	__forceinline int GetTaskPriority(Type ThreadAndIndex)
	{
		return (ThreadAndIndex & TaskPriorityMask) >> TaskPriorityShift;
	}

	__forceinline int GetThreadPriorityIndex(Type ThreadAndIndex)
	{
		int Result = (ThreadAndIndex & ThreadPriorityMask) >> ThreadPriorityShift;
	...
	__forceinline Type SetPriorities(Type ThreadAndIndex, int PriorityIndex, bool bHiPri)
	{
		assert(
			!(ThreadAndIndex & ~ThreadIndexMask) && // not a thread index
...
public:
	...
	**/
	virtual	int GetNumWorkerThreads() = 0;
	...
	*	@param	Callback - function to call prior to shutting down the taskgraph
	**/
	static void BroadcastSlow_OnlyUseForSpecialPurposes(bool bDoTaskThreads, bool bDoBackgroundThreads, std::function<void(ENamedThreads::Type CurrentThread)>& Callback);
};
...
protected:
	...
	**/
	FBaseGraphTask(int InNumberOfPrerequistitesOutstanding)
	...
	{
		checkThreadGraph(LifeStage.Increment() == int(LS_Contructed));
	}
	/**
	...
	**/
	void SetThreadToExecuteOn(ENamedThreads::Type InThreadToExecuteOn)
	{
		...
		checkThreadGraph(LifeStage.Increment() == int(LS_ThreadSet));
	}
	...
	**/
	void PrerequisitesComplete(ENamedThreads::Type CurrentThread, int NumAlreadyFinishedPrequistes, bool bUnlock = true)
	{
		checkThreadGraph(LifeStage.Increment() == int(LS_PrequisitesSetup));
		int NumToSub = NumAlreadyFinishedPrequistes + (bUnlock ? 1 : 0); // the +1 is for the "lock" we set up in the constructor
	...
	{
		checkThreadGraph(LifeStage.Increment() == int(LS_Deconstucted));
	}
...
private:
	...
	**/
	...
	{
		checkThreadGraph(LifeStage.Increment() == int(LS_Executing));
	...
	**/
	...
	{
		checkThreadGraph(LifeStage.Increment() == int(LS_Queued));
...
public:
	...
	**/
	...
	{
		checkThreadGraph(!EventsToWaitFor.size());
	}
...
private:
	...
	*	Destroys myself.
	**/
	virtual void ExecuteTask(std::vector<FBaseGraphTask*>& NewTasks, ENamedThreads::Type CurrentThread) final override
	{
	...
	**/
	TGraphTask(FGraphEventRef InSubsequents, int NumberOfPrerequistitesOutstanding)
	...
	**/
	void SetupPrereqs(const FGraphEventArray* Prerequisites, ENamedThreads::Type CurrentThreadIfKnown, bool bUnlock)
	{
		...
		int AlreadyCompletedPrerequisites = 0;
		if (Prerequisites)
		{
			for (int Index = 0; Index < Prerequisites->size(); Index++)
			{
				assert((*Prerequisites)[Index]);
	...
	**/
	...
	TAlignedBytes<sizeof(TTask), __alignof(TTask)> TaskStorage;
	/** Used to sanity assert the state of the object **/
...
//		assert(InPrerequisite.GetReference());
...
public:
	...
	* @param InDesiredThread - Thread to run on
	**/
	FFunctionGraphTask(std::function<void()>&& InFunction, ENamedThreads::Type InDesiredThread)
	...
	* @return completion handle for the new task
	**/
	static FGraphEventRef CreateAndDispatchWhenReady(std::function<void()> InFunction, const FGraphEventArray* InPrerequisites = NULL, ENamedThreads::Type InDesiredThread = ENamedThreads::AnyThread)
	{
		return TGraphTask<FFunctionGraphTask>::CreateTask(InPrerequisites).ConstructAndDispatchWhenReady(std::move(InFunction), InDesiredThread);
	}
	/**
	* Create a task and dispatch it when the prerequisites are complete
	* @param InTaskDelegate - delegate to execute when the prerequisites are complete
...
public:
	...
	*/
	FGraphEventRef CreatePrerequisiteCompletionHandle(ENamedThreads::Type CurrentThread)
	{
		...
		std::vector<FGraphEvent*> Pending;
		...
		if (Pending.size())
		{
			...
			{
				PendingHandles.push_back(FGraphEventRef(Pending[Index]));
